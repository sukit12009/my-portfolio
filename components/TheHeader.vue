<template>
  <header class="fixed w-full bg-slate-800/80 backdrop-blur-sm z-50 transition-all duration-300"
    :class="{ 'shadow-lg shadow-slate-900/20': scrolled }">
    <div class="container mx-auto px-4 py-4 flex justify-between items-center">
      <div class="text-xl font-bold text-sky-400">
        <span class="text-slate-50">Dev</span>Portfolio
      </div>
      ...
      <button @click="mobileMenuOpen = !mobileMenuOpen" class="md:hidden text-slate-200">
    ...
    <div v-if="mobileMenuOpen" class="md:hidden bg-slate-800 shadow-lg">
      <div class="container mx-auto px-4 py-4">
        <nav class="flex flex-col space-y-4">
          <a v-for="item in navItems" :key="item.id" :href="`#${item.id}`" @click="mobileMenuOpen = false"
